...
from src.search.tfidf import get_custom_ranking, get_sklearn_ranking, get_bm25_ranking, get_tf_ranking

# ---------------------------------------------------------------------------
# App setup
# ---------------------------------------------------------------------------
print(f"[STARTUP] Loading data from: {os.path.abspath('data/scraper_results.json')}")
...
def _load_data():
    ...
    print(f"[STARTUP] ALL_DOC_IDS: {len(all_doc_ids)}")
    print(f"[STARTUP] Sample URLs: {list(all_doc_ids)[:3]}")
    pub_lookup  = {p.get("url"): p for p in publications if p.get("url")}
    print(f"[STARTUP] Loaded {len(publications)} publications")
...
def search_boolean(
    ...
    filtered      = _apply_filters(urls_list, year, doc_type, None, None, year_from=year_from, year_to=year_to)
    filtered      = _filter_by_fields(filtered, q, field_list)
    paginated     = _paginate(filtered, page, page_size)
    hl_tokens     = _query_surface_tokens(q)
...
def search_proximity(
    term1: str = Query(...),
    term2: str = Query(...),
    distance: int = Query(5, ge=1, le=50),
    fields: Optional[str] = Query(None),
    year: Optional[int] = Query(None),
    doc_type: Optional[str] = Query(None),
    page: int = Query(1, ge=1),
...
def search_author(
    ...
    return {"query_author": name, "total": len(matched), "page": page, "page_size": page_size, "results": paginated}
...
def get_document(url: str = Query(...)):
    pub = PUB_LOOKUP.get(url)
    if pub is None:
...
# ── 8. All publications (for stats page) ─────────────────────────────────────

@app.get("/publications", tags=["Info"])
def get_all_publications(
    page: int = Query(1, ge=1),
    page_size: int = Query(100, ge=1, le=200),
):
    """Returns all publications for client-side statistics computation."""
    if not PUBLICATIONS:
        return {"total": 0, "results": []}
    paginated = _paginate(PUBLICATIONS, page, page_size)
    results = [_build_result(p.get("url", ""), None, p) for p in paginated]
    return {"total": len(PUBLICATIONS), "results": results}
...
def debug_data():
    return {
        ...
        "all_doc_ids_count": len(ALL_DOC_IDS),
...
def debug_preprocess(text: str = Query(...)):
